...
class UsecasesSpec extends FlatSpec with Matchers with MockFactory {
  ...
  it should "match text" in {
    (mockedStatus1.getText _).expects().returning("test1 #Coronavirus").anyNumberOfTimes()
    filteredList.headOption.getOrElse(fail()).getText shouldBe "test1 #Coronavirus"
  }
  ...
  it should "contain hashtag #Coronavirus" in {
    mappedList2.headOption.getOrElse(fail())._1 shouldBe "#Coronavirus"
  }
  ...
  it should "match context" in {
    mappedList3.headOption.getOrElse(fail()) shouldBe ("#Coronavirus",(1,2.0))
  }
  ...
  it should "work" in {
    Usecases.countsAveSentimentScore("#Coronavirus",(2,6.0)) shouldBe ("#Coronavirus",(2,3.0))
  }
